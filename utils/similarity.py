...
def simple_sentence_similarity(s1, s2, documents=None):
    # Tokens
    s1_tokens = stringutils.tokenize_twitter(s1)
    s2_tokens = stringutils.tokenize_twitter(s2)

    # Stems
    s1_stems = set(stringutils.stem(s1_tokens))
    s2_stems = set(stringutils.stem(s2_tokens))
	
    print s1_stems,s2_stems
    return float(len(s1_stems & s2_stems)) / float(len(s1_stems | s2_stems))

def main(sentences, simple=True):
